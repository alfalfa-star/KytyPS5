...
static vk::Device VulkanCreateDevice(GraphicContext&                 graphics,
	...
	// Descriptor tables of V#s picked by a runtime key index the storage buffer array
	// dynamically (see IndirectBufferScope); every desktop implementation exposes this.
	EXIT_NOT_IMPLEMENTED(supported_features2.features.shaderStorageBufferArrayDynamicIndexing !=
	                     VK_TRUE);
	device_features.shaderStorageBufferArrayDynamicIndexing = VK_TRUE;
	// Double-precision VALU (V_*_F64) needs native doubles in SPIR-V; a device without them
	// only fails when a shader actually uses one.
	device_features.shaderFloat64 = supported_features2.features.shaderFloat64;
	if (device_features.shaderFloat64 != VK_TRUE) {
		LOGF("shaderFloat64 is not supported: shaders using V_*_F64 will fail to compile\n");
	}
