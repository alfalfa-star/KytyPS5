...
IR::Value Translator::ReadOperand(const Decoder::Operand& operand, IR::Type type) {
	...
	if (type == IR::Type::F64) {
		const auto pair = ReadU32Pair(operand);
		auto value = ir.Emit(IR::ValueOpcode::BitCastF64U64, {ir.ConstructU64(pair[0], pair[1])});
		if (operand.absolute) {
...
void Translator::AddBranchCondition(const CFG::BasicBlock& source, IR::BlockInfo& info) {
	...
	// EXEC and VCC are invocation-local Boolean masks; the condition is this invocation's bit and
	// the emitter reduces it over the subgroup, since the wave branches as a whole.
	IR::U1 condition;
